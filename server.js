...
cron.schedule('* * * * *', () => {
  ...
  console.log('Revisando recordatorios para:', currentHour, today);

  // Buscar recordatorios para la hora actual exacta
  db.all(
    `SELECT * FROM reminders 
     WHERE hour = ? 
     AND (startDate IS NULL OR startDate <= ?) 
     AND (endDate IS NULL OR endDate >= ?)`,
    [currentHour, today, today],
    (err, rows) => {
      if (err) {
      ...
      console.log(`Encontrados ${rows.length} recordatorios para ${currentHour}`);
